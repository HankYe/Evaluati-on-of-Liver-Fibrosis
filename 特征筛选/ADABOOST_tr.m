...
aggClassEst=zeros(sample_n,1);

% hwait=waitbar(0,'耐心等待<<<<<<<<');
for turn=1:no_of_hypothesis
    tic
    turn
%     str=['正在运行中',num2str(turn*100/(no_of_hypothesis)),'%'];
%     waitbar(turn/(no_of_hypothesis),hwait,str);
    ...
    %%%aggClassEst = aggClassEst + L(:,1)*adaboost_model.weights(turn)
    ...
    toc
%     Error_rate(turn)=error_rate;
%     Turn(turn)=turn;
end
% plot(Turn,Error_rate)
% close(hwait)
% Normalization
